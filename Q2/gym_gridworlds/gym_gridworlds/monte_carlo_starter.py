...
def monte_carlo_off_policy_control(env, num_episodes, seed, noise=0.0,gamma=0.9, epsilon_start=0.15, epsilon_min=0.01):
    ...
    for episode in range(num_episodes):
        ...
        # Backward pass with weighted IS
        G = 0.0
        W = 1.0
        for (s, a, r) in reversed(episode_data):
            ...
            pi_prob = get_target_prob(s, a, epsilon)
...
if __name__ == '__main__':
    ...
    for noise in noise_values:
        ...
        for seed in range(num_seeds):
            ...
            q_values, policy, episode_rewards = monte_carlo_off_policy_control(
        ...
        # plot with proper naming
        plot_path = f"plots/monte_carlo_reward_curve_({noise})_plot.png"
        plt.figure()
        plt.plot(smoothed_rewards, label=f"Noise={noise}")
        plt.xlabel("Episodes")
        plt.ylabel("Average Return (Moving Avg 100)")
        plt.title(f"Average Reward vs Episodes (noise={noise})")
        plt.legend()
        plt.savefig(plot_path)
        plt.close()

        # evaluate best policy only once (100 runs)
        eval_rewards = []
        for _ in range(100):
            state, _ = env.reset()
        ...
        # gif with proper naming
        gif_path = f"gifs/monte_carlo_gif_({noise}).gif"
        generate_policy_gif(env, best_policy, filename=gif_path)

        # collect results for JSON
        final_results[f"MC_ImportanceSampling({noise})"] = eval_rewards

        env.close()

    # update JSON at the end (mean/std are computed inside)
